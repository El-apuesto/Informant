-- ============================================
-- n4mint AI Transcription Platform - Fixed Database Schema
...
-- ============================================
-- SECURE VIEWS (Fixed - No SECURITY DEFINER)
-- ============================================

-- User subscription summary view (without SECURITY DEFINER)
...
WHERE u.id = auth.uid() -- Security: Users can only see their own data
ORDER BY u.created_at DESC;

-- User usage statistics view (without SECURITY DEFINER)
...
-- ============================================
-- SECURE FUNCTIONS FOR ADMIN ACCESS
-- ============================================

-- Function for admin to get all user subscription summaries
...
SECURITY DEFINER -- Only for admin/service role
LANGUAGE sql
STABLE
AS $$
    SELECT 
...
-- Function for admin to get all usage statistics
...
RETURNS TABLE (
    ...
    failed_jobs bigint,
    minutes_this_month decimal(10,2),
    cost_this_month_cents integer
)
SECURITY DEFINER -- Only for admin/service role
LANGUAGE sql
STABLE
...
-- ============================================
-- RLS POLICIES FOR SECURE FUNCTIONS
-- ============================================

-- Only allow service_role to access admin functions
CREATE POLICY "Only service role can access subscription summary" ON get_all_subscription_summary FOR ALL USING (auth.role() = 'service_role');
CREATE POLICY "Only service role can access usage stats" ON get_all_usage_stats FOR ALL USING (auth.role() = 'service_role');
...
-- 3. Usage tracking for billing limits
-- 4. Job processing for transcription
-- 5. Game scoring and leaderboards
-- 6. Proper security with RLS
-- 7. Performance optimization with indexes
-- 8. Automated triggers for timestamps
-- 9. Secure views without SECURITY DEFINER issues
-- 10. Admin functions for service role access
